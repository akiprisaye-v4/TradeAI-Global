...
const AMAZON_DOMAINS = { FR: "amazon.fr", DE: "amazon.de", UK: "amazon.co.uk", US: "amazon.com", IT: "amazon.it", ES: "amazon.es" };
const TRENDS_GEO = { FR: "FR", DE: "DE", UK: "GB", US: "US", IT: "IT", ES: "ES" };
...
function SelectField({ label, value, onChange, options }) {
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      <select id={id} value={value} onChange={e => onChange(parseInt(e.target.value))} style={{ width: "100%", background: "#1C2128", border: "1px solid #30363D", borderRadius: 7, padding: "9px 10px", color: "#E6EDF3", fontSize: 13, outline: "none", cursor: "pointer" }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 14 }}>
      <div>
        <Section title="🏪 Produit & Marché">
          <InputField label="Nom du produit" value={p.name} onChange={v => u("name", v)} prefix="" />
          <SelectField label="Marketplace" value={p.marketplace} onChange={v => u("marketplace", v)} options={Object.entries(MARKETPLACES).map(([k, v]) => ({ label: v.label, value: k }))} />
...
const defaultProduct = (name = "Produit 1") => ({
  ...
  ads: 1.50, otherCosts: 0, units: 100,
  supplierLeadDays: 30, initialOrderUnits: 500, rampMonths: 3,
});

const TABS = ["apropos", "idees", "calculateur", "cogs", "pricing", "comparateur", "historique"];
const TAB_LABELS = {
  apropos: "ℹ️ À propos",
  idees: "💡 Idées",
  calculateur: "📊 Calcul",
